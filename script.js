document.addEventListener('DOMContentLoaded', () => {
  ...
  function fitDecorTextSize(text, widthMm, heightMm, family, weight, italic) {
    ...
    while (lo <= hi) {
      ...
      if (probe.offsetHeight <= (budgetH + 0.5) && probe.scrollWidth <= (budgetW + 3.0)) {
        best = mid;
        lo = mid + 1;
      } else {
        hi = mid - 1;
  ...
  // === Canvas 2D двигатель замера текста наименований (Решение 1) ===
  // Полностью автономен от DOM, не вызывает reflow/layout-coalescing в Firefox,
  // иммунен к масштабу страницы (browser zoom) и DPI экрана.
  let __titleMeasureCanvas = null;
  let __titleMeasureCtx = null;
  function getTitleMeasureCtx() {
    if (!__titleMeasureCtx) {
      try {
        __titleMeasureCanvas = document.createElement('canvas');
        __titleMeasureCanvas.width = 1;
        __titleMeasureCanvas.height = 1;
        __titleMeasureCtx = __titleMeasureCanvas.getContext('2d');
      } catch (e) {
        console.warn('Canvas 2D context creation failed:', e);
        __titleMeasureCtx = null;
      }
    }
    return __titleMeasureCtx;
  }

  function formatCanvasFont(fontSizePt, family, weight, isItalic) {
    const sizePx = fontSizePt * (96 / 72);
    const style = isItalic ? 'italic' : 'normal';
    const w = weight || '800';
    const fam = family || 'Arial, sans-serif';
    return `${style} ${w} ${sizePx}px ${fam}`;
  }

  // Замер геометрии и переноса строк наименования через Canvas 2D.
  function measureTitleLayout(text, fontSizePt, family, weight, isItalic, budgetW, budgetH) {
    const formatted = formatSmartTitle(text);
    if (!formatted) {
      return { fits: true, linesCount: 0, totalHeight: 0, maxLineWidth: 0 };
    }

    const ctx = getTitleMeasureCtx();
    if (!ctx) {
      // Резервный замер через DOM probe, если Canvas не поддерживается
      const probe = getTitleProbe(budgetW);
      probe.style.fontFamily = family || 'Arial, sans-serif';
      probe.style.fontWeight = weight || '800';
      probe.style.fontStyle = isItalic ? 'italic' : 'normal';
      probe.style.fontSize = `${fontSizePt}pt`;
      probe.textContent = formatted;
      const pH = probe.offsetHeight;
      const pW = probe.scrollWidth;
      return {
        fits: (pH <= (budgetH + 1.0)) && (pW <= (budgetW + 3.0)),
        linesCount: 1,
        totalHeight: pH,
        maxLineWidth: pW
      };
    ...
    const paragraphs = formatted.split(/\r?\n/);

    let totalLines = 0;
    let maxLineWidth = 0;
    let anyCharExceedsBudget = false;
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (/^ +$/.test(tok)) {
          if (!curLine) {
            ...
            const testW = ctx.measureText(curLine + tok).width;
            if (testW <= (budgetW + 0.5)) {
              curLine += tok;
            } else {
              totalLines += 1;
              maxLineWidth = Math.max(maxLineWidth, ctx.measureText(curLine.trimEnd()).width);
              curLine = '';
            }
          }
        } else {
          if (!curLine) {
            ...
            if (tokW <= (budgetW + 0.5)) {
              ...
              // Слово длиннее ширины строки: перенос по символам (word-break: break-word)
              let chunk = '';
              for (const ch of tok) {
                const chW = ctx.measureText(ch).width;
                if (chW > (budgetW + 3.0)) {
                  anyCharExceedsBudget = true;
                }
                const testChunk = chunk + ch;
                if (ctx.measureText(testChunk).width <= (budgetW + 0.5)) {
                  chunk = testChunk;
                } else {
                  if (chunk) {
                    totalLines += 1;
    ...
    const fits = !anyCharExceedsBudget &&
      (totalHeight <= (budgetH + 1.0)) &&
      (maxLineWidth <= (budgetW + 3.0));
  ...
  function fitTitleSize(text, family, weight, itemIdx = activePreviewIndex) {
    ...
    const fam = family || 'Arial, sans-serif';
    const wgt = weight || '800';

    let best = floor;
    let lo = floor, hi = TITLE_FIT_MAX;
    while (lo <= hi) {
      const mid = Math.round(((lo + hi) / 2) * 2) / 2;
      const layout = measureTitleLayout(text, mid, fam, wgt, isIt, budgetW, budgetH);
      if (layout.fits) {
        best = mid;
        lo = mid + 0.5;
      } else {
  ...
  function fitTitleSizeForPreset(text, preset) {
    ...
    while (lo <= hi) {
      const mid = Math.round(((lo + hi) / 2) * 2) / 2;
      const layout = measureTitleLayout(text, mid, family, weight, isIt, budgetW, budgetH);
      if (layout.fits) {
        best = mid;
        lo = mid + 0.5;
      } else {
  ...
  function checkTextOverflow() {
    ...
    try {
      ...
      const isIt = (templateFonts && templateFonts.titleItalic) || (titleItalic && titleItalic.checked);
      const layout = measureTitleLayout(titleText, currentSize, family, weight, isIt, budgetW, budgetH);
      if (!layout.fits) {
        isOverflowing = true;
      }
    } catch (err) {
      console.warn('checkTextOverflow layout error:', err);
    }
      ...
      for (const sel of selected) {
        ...
        items.forEach((it, idx) => {
          ...
          try {
            ...
            const layout = measureTitleLayout(it.title || '', currentSize, family, weight, isIt, budgetW, budgetH);
            if (!layout.fits) {
              issues.push({
                type: 'warn',
  ...
  function updateDebugToolbar() {
    ...
    try {
      ...
      // 7. Проба текста (Canvas замер)
      if (typeof fitPt === 'number' && fitPt > 0) {
        try {
          const layout = measureTitleLayout(rawTitle, fitPt, fam, wgt, isIt, budgetW, budgetH);
          if (probeEl) {
            probeEl.textContent = `H:${Math.round(layout.totalHeight)}px, W:${Math.round(layout.maxLineWidth)}px (${layout.linesCount}стр, Canvas)`;
          }
        } catch (_) {
    ...
    dbgCopyBtn.addEventListener('click', () => {
      try {
        ...
        const info = [
          ...
          'Сборка: v1.9.8-CANVAS [09.09 16:35]',
          ...
          `Замер Canvas: ${document.getElementById('dbgProbe')?.textContent || '—'}`,
