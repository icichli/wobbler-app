document.addEventListener('DOMContentLoaded', () => {
  ...
  const templateItems = {};
  function freshItem() {
    return { title: '', price: '', subtitle: '', subtitleManual: false, digit: '', count: 1 };
  }
  ...
  const builtInPresets = {
    ...
    yellow_tag: {
      ...
      decorBottomFontSize: 14,
      decorBottomHeight: 12,
      // Зазор между ценниками на листе А4 — 0,5 мм.
      gapMm: 0.5,
      titleFitFloor: 5.5,
      labelPos: { title: { x: -0.4, y: 4.75 }, subtitle: { x: -4.9, y: -0.3 }, price: { x: 0.2, y: 1.7 }, priceDigits: [{ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }], currency: { x: 0, y: 0 } }
    },
    novy_vkus: {
      name: 'Новый вкус',
      widthCm: 6.5,
  ...
  function updateItemsStatsBadge() {
    ...
    if (badge) {
      ...
      badge.title = `Всего к печати: ${totalCards} ценников (${filledCount} уникальных товаров, на лист влезает ${grid.maxCount} шт., требуется ${totalCards > 0 ? pages : 0} листов А4)`;
    }
  }
  ...
  // Клавиатурная навигация в таблице товаров: Tab и стрелки ArrowUp / ArrowDown
  function handleTableNavKeyDown(e, inputEl, colClass, rowIndex) {
    const list = inputEl.closest('.items-list');
    if (!list) return;

    // Tab на поле цены (или цифры) переходит сразу к названию следующей строки (пропуская кнопки ❌, 🖼, 🎨, ✕)
    ...
    // Стрелка вниз (в однострочном поле или если курсор в самом конце)
    if (e.key === 'ArrowDown') {
    ...
    // Стрелка вверх (в однострочном поле или если курсор в самом начале)
    if (e.key === 'ArrowUp') {
      const val = inputEl.value || '';
      const isSingleLine = !val.includes('\n');
  ...
  function createItemRow(i) {
    ...
    row.innerHTML = `
      <div class="item-move-wrap" title="Порядок товара">
      ...
      <div class="item-qty-wrap" title="Тираж на листе А4"><input type="number" class="item-qty-input" min="1" max="99" value="${countVal}" data-index="${i}" title="Тираж копий на листе А4"></div>
    ...
    // Иконка-кнопка 🏷️ — старая цена и скидка % для конкретного товара
  ...
  function moveItemRow(fromIdx, delta) {
    ...
    if (typeof pushHistoryState === 'function') pushHistoryState('Перемещение товара');
  ...
  function sortItems(criterion) {
    if (!itemsData || itemsData.length <= 1) return;
    if (typeof pushHistoryState === 'function') pushHistoryState('Сортировка товаров');
    ...
    } else if (criterion === 'filled_first') {
    ...
    updatePreview();
    scheduleSessionSave();
    if (typeof showToast === 'function') showToast('Список товаров отсортирован', 'success', 2000);
  }
  ...
  // Очистка ввода цены от текстового мусора и незначащих нулей
  function cleanPriceInput(val) {
    if (!val) return '';
    let s = String(val).trim();
    s = s.replace(/,/g, '.');
    s = s.replace(/([0-9])o([0-9])/gi, '$10$2');
    s = s.replace(/\s*(?:руб(?:л[ея]й)?|р(?:\.|\b)|₽)\s*/gi, '');
    s = s.replace(/\.00(?!\d)/, '');
  ...
  // Calculate maximum fitting wobblers on A4 (поля печати: 10 мм сверху, по 4 мм
  ...
  function calcA4Grid(wMm, hMm) {
    const mTop = 10, mBottom = 4, mSide = 4; // верх 10 мм, низ/бока 4 мм
    const g = gapMm();
    const pageW = 210 - mSide * 2;       // 202
    const pageH = 297 - mTop - mBottom;  // 283
  ...
  function applyItemToClone(clone, item, titleOffsetYVal, priceOffsetYVal) {
    ...
    if (pElem) {
      ...
      renderPriceElement(pElem, String((item && item.price) || ''), lp.priceDigits, priceCentsSup);
    }
    if (curr) {
    ...
    for (const sel of selected) {
      ...
      if (items.length === 1) {
        for (let i = 0; i < copiesNum; i++) {
          queue.push({ item: items[0], preset, templateKey: sel.key, itemIndex: 0 });
        }
      } else {
        if (sel.copies === 'auto' || !sel.copies) {
          for (let i = 0; i < items.length; i++) {
            queue.push({ item: items[i], preset, templateKey: sel.key, itemIndex: i });
          }
        } else {
    ...
    for (let i = 0; i < pageItems.length; i++) {
      ...
      if (isMultiMode) {
        if (origIndex !== -1) {
          ...
          itemWrapper.title = `№${origIndex + 1}: ${item.title || 'Без названия'}${item.price ? ' — ' + item.price + ' ₽' : ''}\n👉 Нажмите, чтобы открыть в редакторе`;
  ...
  function getItemsForTemplate(key) {
    ...
    if (isActive && !isMultiMode) {
      ...
      if (title) {
        ...
        return [{ title, price, subtitle, digit, labelPos: singleLabelPos }];
      }
      return [];
    }
  ...
  async function downloadPrintAreaPdf(isMulti = false) {
    ...
    exportWrapper.style.cssText = 'display: block; position: absolute; left: 0; top: 0; width: 210mm; background: #ffffff; z-index: 999999; margin: 0; padding: 0; box-sizing: border-box; overflow: visible;';
  ...
  function renderMultiPrintTemplates() {
    ...
    multiPrintTemplatesEl.innerHTML = TEMPLATE_KEYS.map(k => {
      ...
      return `<div class="multi-print-row-item ${isEmpty ? 'is-empty' : ''}">
        ...
        <span class="mpi-filled ${isEmpty ? 'is-empty' : ''}">${filled} тов.</span>
  ...
  function updateMultiPrintSummary() {
    ...
    for (const sel of selected) {
      ...
      if (!items.length) continue;
      const copiesNum = (sel.copies === 'auto' || !sel.copies) ? 1 : (parseInt(sel.copies, 10) || 1);
      const templateTotal = items.length * copiesNum;
      count += templateTotal;
    }
    if (count === 0) {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'z' && !e.shiftKey) {
      ...
      const isInput = activeEl && ['INPUT', 'TEXTAREA'].includes(activeEl.tagName) && !activeEl.classList.contains('items-search-input');
      if (!isInput) {
        e.preventDefault();
        undoAction();
      }
    ...
    if (e.key === 'Escape') {
      ...
      if (typeof sidebarSortMenu !== 'undefined' && sidebarSortMenu && sidebarSortMenu.style.display === 'block') {
        sidebarSortMenu.style.display = 'none';
      }
      if (typeof idrSortMenu !== 'undefined' && idrSortMenu && idrSortMenu.style.display === 'block') {
  ...
  document.addEventListener('click', (e) => {
    if (sidebarSortMenu && sidebarSortMenu.style.display === 'block' && !sidebarSortMenu.contains(e.target) && e.target !== sidebarSortBtn) {
      sidebarSortMenu.style.display = 'none';
    }
    if (idrSortMenu && idrSortMenu.style.display === 'block' && !idrSortMenu.contains(e.target) && e.target !== idrSortBtn) {
      idrSortMenu.style.display = 'none';
    }
  });
  ...
  function checkTextOverflow() {
    ...
    try {
      ...
      if (probe.offsetHeight > budgetH + 1 || probe.scrollWidth > budgetW + 1) {
        isOverflowing = true;
      }
    } catch (err) {
  ...
  function runPreflightAudit() {
    ...
    if (isMultiJob) {
      ...
      for (const sel of selected) {
        ...
        if (items.length === 1) {
          totalCards += copiesNum;
        } else {
          totalCards += (sel.copies === 'auto' || !sel.copies) ? items.length : (items.length * copiesNum);
        }

        // Геометрические границы ценника шаблона для зонда переполнения
        const wMm = (preset.widthCm || 6.5) * 10;
        ...
        items.forEach((it, idx) => {
          // 1. Проверка цен
          const priceVal = (it.price || '').trim();
          if (!priceVal) {
            issues.push({
              type: 'err',
              msg: `[${tplName}] Товар №${idx + 1} «${it.title}»: не указана цена!`,
            ...
            issues.push({
              type: 'warn',
              msg: `[${tplName}] Товар №${idx + 1} «${it.title}»: цена указана как 0 ₽`,
              actionType: 'focusPrice',
              templateKey: tplKey,
              itemIndex: idx
            });
          ...
          try {
            ...
            if (probe.offsetHeight > budgetH + 1 || probe.scrollWidth > budgetW + 1) {
              issues.push({
                type: 'warn',
                msg: `[${tplName}] Товар №${idx + 1} «${it.title}»: текст названия выходит за границы ценника`,
                actionType: 'refit',
                templateKey: tplKey,
                itemIndex: idx
              });
          ...
          try {
            ...
            if (!titleSh || titleSh === 'none' || titleSh === '0px 0px 0 #000000') {
              ...
              if (cr < 2.8) {
                issues.push({
                  type: 'warn',
                  msg: `[${tplName}] Товар №${idx + 1} «${it.title}»: низкая контрастность названия (${cr.toFixed(1)}:1). Рекомендуется включить тень.`,
                  actionType: 'contrast',
                  templateKey: tplKey,
                  itemIndex: idx,
      ...
      const totalSheets = Math.max(1, Math.ceil(totalCards / 24));
    ...
    if (filledItems.length === 0) {
      ...
      return { hasErrors: true, hasWarnings: false, issues, totalCards: 0, totalSheets: 0, sheetCapacity, filledItemsCount: 0 };
    }
    ...
    filledItems.forEach(({ it, idx }) => {
      ...
      if (!priceVal) {
        issues.push({
          type: 'err',
          msg: `Товар №${idx + 1} «${it.title}»: не указана цена!`,
          actionType: 'focusPrice',
        ...
        issues.push({
          type: 'warn',
          msg: `Товар №${idx + 1} «${it.title}»: цена указана как 0 ₽`,
          actionType: 'focusPrice',
          templateKey: activeKey,
          itemIndex: idx
        });
    ...
    filledItems.forEach(({ it, idx }) => {
      ...
      try {
        ...
        if (probe.offsetHeight > budgetH + 1 || probe.scrollWidth > budgetW + 1) {
          issues.push({
            type: 'warn',
            msg: `Товар №${idx + 1} «${titleText}»: текст названия выходит за границы ценника`,
            actionType: 'refit',
            templateKey: activeKey,
            itemIndex: idx
          });
      ...
      try {
        ...
        if (!titleSh || titleSh === 'none' || titleSh === '0px 0px 0 #000000') {
          ...
          if (cr < 2.8) {
            issues.push({
              type: 'warn',
              msg: `Товар №${idx + 1} «${it.title}»: низкая контрастность названия (${cr.toFixed(1)}:1, WCAG советует ≥ 3:1). Рекомендуется включить тень.`,
              actionType: 'contrast',
              templateKey: activeKey,
              itemIndex: idx,
              contrastRatio: cr.toFixed(1)
    ...
    if (isMultiMode) {
      ...
      if (singleRepeatCount && singleRepeatCount.value === 'auto') {
        totalCards = sheetCapacity;
      } else {
        totalCards = Math.min(parseInt(singleRepeatCount ? singleRepeatCount.value : 24, 10) || 1, sheetCapacity);
      }
    }

    const totalSheets = Math.ceil(totalCards / sheetCapacity);
    const remainder = totalCards % sheetCapacity;
    if (remainder !== 0 && totalSheets > 0) {
      const emptySpots = sheetCapacity - remainder;
      issues.push({
        type: 'info',
        msg: `На последнем листе остаётся ${emptySpots} ${emptySpots === 1 ? 'свободное место' : 'свободных мест'} (${remainder} из ${sheetCapacity} занято).`,
        actionType: 'info'
      });
    }
  ...
  function showPreflightModal(onProceed) {
    ...
    if (preflightSummaryBanner) {
      ...
      if (audit.hasErrors) {
        preflightSummaryBanner.innerHTML = `❌ <b>Обнаружены ошибки:</b> ${audit.issues.filter(i => i.type === 'err').length} критических замечаний. Исправьте их перед печатью.`;
      } else if (audit.hasWarnings) {
        preflightSummaryBanner.innerHTML = `⚠️ <b>Есть предупреждения:</b> макет готов, но найдено ${audit.issues.filter(i => i.type === 'warn').length} замечаний.`;
      } else {
        preflightSummaryBanner.innerHTML = `✅ <b>Всё отлично!</b> Все ценники (${audit.totalCards} шт. на ${audit.totalSheets} стр.) проверены и готовы к печати.`;
      }
    }
    ...
    if (preflightList) {
      ...
      if (!audit.issues.length) {
        ...
        audit.issues.forEach(issue => {
          ...
          const badgeClass = issue.type === 'err' ? 'err' : (issue.type === 'warn' ? 'warn' : 'ok');
  ...
  function updatePreflightPill() {
    ...
    try {
      ...
      pill.classList.remove('status-ready', 'status-warning', 'status-error');
      if (audit.hasErrors) {
        pill.classList.add('status-error');
        pillText.textContent = `❌ ${errCount} ош.`;
        pill.title = `Предпечатный статус: обнаружено ${errCount} ошибок! Нажмите, чтобы открыть аудит.`;
      } else if (audit.hasWarnings) {
        pill.classList.add('status-warning');
        pillText.textContent = `⚠️ ${warnCount} зам.`;
        pill.title = `Предпечатный статус: ${warnCount} замечаний. Нажмите, чтобы открыть аудит.`;
      } else {
        pill.classList.add('status-ready');
        pillText.textContent = `✓ ${audit.totalCards} шт. (${audit.totalSheets} л.)`;
        pill.title = `Готово к печати: ${audit.totalCards} шт. на ${audit.totalSheets} лист(ах). Нажмите, чтобы открыть аудит.`;
      }
    } catch (_) {}
  }
