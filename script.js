document.addEventListener('DOMContentLoaded', () => {
  ...
  function runPreflightAudit() {
    ...
    if (isMultiJob) {
      ...
      for (const sel of selected) {
        ...
        items.forEach((it, idx) => {
          ...
          // 1. Проверка цен с учётом тиража
    ...
    if (filledItems.length === 0) {
      ...
      return { hasErrors: true, hasWarnings: false, issues, totalCards: 0, totalSheets: 0, sheetCapacity, filledItemsCount: 0, isMultiJob: false, copiesDetail: '0 ценников' };
    }

    // 1. Проверка цен и тиража
