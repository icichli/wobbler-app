document.addEventListener('DOMContentLoaded', () => {
  ...
  function filterExtraBgOptionsForActiveTemplate() {
    ...
    Array.from(group.querySelectorAll('option')).forEach(opt => {
      ...
      if (isRyba) {
        ...
        const isSnekiBg = isOstryi || opt.value.includes('kalmar') || opt.value.includes('myaso') || opt.value.includes('syr') || opt.value.includes('ryb_solomka');
  ...
  function refreshAutoBgBtns() {
    document.querySelectorAll('.autobg-toggle').forEach(b => {
      b.classList.toggle('active', autoBgByTitleEnabled);
      const isFull = b.classList.contains('autobg-toggle-full');
      const stateText = autoBgByTitleEnabled ? 'ВКЛ' : 'ВЫКЛ';
      const actionText = autoBgByTitleEnabled ? 'клик — выключить' : 'клик — включить';
      b.title = `Автоподстановка фона по названию товара: ${stateText} (${actionText})`;
      if (isFull) {
        b.innerHTML = `✨ Авто-фон: <b>${stateText}</b>`;
      }
    });
  }

  function toggleAutoBgByTitle() {
    autoBgByTitleEnabled = !autoBgByTitleEnabled;
    try {
      localStorage.setItem(AUTOBG_LS_KEY, autoBgByTitleEnabled ? '1' : '0');
    } catch (e) { }
    refreshAutoBgBtns();
    if (autoBgByTitleEnabled) {
  ...
  document.querySelectorAll('.autobg-toggle').forEach(b => {
    b.addEventListener('click', toggleAutoBgByTitle);
  });
  refreshAutoBgBtns();
  ...
  function getAutoBgForTitle(title, templateKey) {
    ...
    if (templateKey === 'sneki') {
      ...
      // 5. Острое
      if (
        t.includes('остр') || t.includes('чили') ||
        t.includes('халапень') || t.includes('табаско') ||
        t.includes('васаби') || t.includes('spicy')
      ) {
  ...
  function applyAutoBgToAllItems() {
    if (!autoBgByTitleEnabled) return;
    const templateKey = (activeTemplateRef && activeTemplateRef.kind === 'builtin') ? activeTemplateRef.key : '';
    if (templateKey !== 'sneki' && templateKey !== 'alaska_dots') return;

    const isMultiMode = document.querySelector('input[name="printMode"]:checked')?.value === 'multi';
    if (isMultiMode) {
      itemsData.forEach((item, idx) => {
        if (item && !item.bgManual && item.title) {
  ...
  applyPasteBtn.addEventListener('click', () => {
    ...
    // Автоподстановка фона по наименованиям
    applyAutoBgToAllItems();
  ...
  function renderSheetPreview(wMm, hMm) {
    ...
    for (let i = 0; i < pageItems.length; i++) {
      ...
      if (isMultiMode) {
        const origIndex = itemsData.indexOf(item);
        if (origIndex !== -1) {
          itemWrapper.style.cursor = 'pointer';
          itemWrapper.title = `Кликните, чтобы открыть товар #${origIndex + 1} в основном предпросмотре`;
          if (origIndex === activePreviewIndex) {
            itemWrapper.style.outline = '2px solid var(--accent-blue)';
  ...
  const EMBEDDABLE_BGS = ['dots_bg.jpg', 'ryba_bg.jpg', 'yellow_bg.jpg', 'sneki_bg.jpg', 'sneki_5_bg.jpg', 'sneki_digit_bg.jpg', 'sort_nedeli_bg.jpg', 'sort_nedeli_yellow.jpg', 'korona_a5_bg.jpg', 'korona_a5_orange.jpg', 'korona_a5_blue.jpg', 'korona_a5_green.jpg', 'korona_a5_red.jpg', 'a5.jpg', 'a5_orange.jpg', 'a5_red.jpg', 'a5_blue.jpg', 'a5_green.jpg', 'aktsiya_a5_bg.jpg', 'kalmar.png', 'myaso.png', 'syr.png', 'ryb_solomka.png', 'ostryi.png'];
  ...
  if (downloadPdfBtn) downloadPdfBtn.addEventListener('click', () => {
    const jobMode = getPrintJobVal();
    downloadPrintAreaPdf(jobMode === 'multi');
  });
